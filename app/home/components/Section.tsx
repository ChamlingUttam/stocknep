...
import { useTestimonial } from '@/hooks/testimonial'
...
// //     setActiveTestimonial(testimonials?.[0]?.description ?? "")
// //   // }

//    console.log("testimonial:", testimonials)
//      console.log("name is:", testimonials?.[0]?.name)
// }, [testimonials])
      ...
      <div className='w-full flex flex-col lg:flex-row items-stretch justify-between px-6 lg:px-30 gap-5 pb-10'>
        ...
        <div className='w-full lg:w-9/12 border border-gray-400 rounded-lg p-6 lg:p-8 flex flex-col justify-center'>
         ...
         {/* {activeTestimonial?.description} */}
           {/* {activeTestimonial || testimonials?.[0]?.description} */}
           {activeTestimonial?.description ||
        ...
        <div className='w-full lg:w-3/12 border border-gray-400 rounded-lg p-6 flex flex-col gap-4 justify-center'>
          {testimonials?.map((t) => (
            <div
              ...
              // }`}
               className="flex flex-col border-b border-gray-200 pb-3 cursor-pointer transition-colors opacity-100" 
              onClick={() => setActiveTestimonial(t)}
            >
              <h3 className='text-[#033773] font-medium'>{t.name}</h3>
              <p className='text-[#0057FB] text-sm'>@{t.position}</p>
            </div>
          ))}
        </div>
      </div>
